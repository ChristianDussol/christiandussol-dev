...
I wrote this from the allocation side: getting the right accelerator to the right workload. A KubeCon Japan talk pointed the same primitive at a question I had not thought to ask it.

Faseela K and Sunyanan Choochotkaew proposed that vendors describe power characteristics in the ResourceSlice, which turns DRA from an allocation API into Kubernetes' contract with hardware vendors. Same mechanism, different question: not which device fits, but what it costs to run.

→ [Conference highlight: Kubernetes and the missing half of energy-aware scheduling](../../field-notes.md)
